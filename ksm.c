...
struct rmap_item {
	...
	unsigned long oaddress;		/* HZ:address with unchanged low bits */
	...
	struct hlist_node gfnhlist;     /* HZ:link into hlist of rmap_items hanging off that gpa_node */
	struct list_head link;		/* HZ: */
...
/* Milliseconds ksmd should sleep between batches inside hotzone*/
static unsigned int ksm_thread_sleep_millisecs_hotzone = 2;
...
static void remaining_show(void)
{
	...
	printk("Start dumping remaining information:\n");
	list_for_each_entry(rmap_item, &remaining_rmap, link) {
		printk("VM#%lu, remain GFN = %lu, count =%d\n", rmap_item->number, rmap_item->gfn, hot_table[rmap_item->gfn]);
	}
	printk("=========Finish dumping remaining information.=========\n");
}
...
*/
		...
		/* printk("cmp_and_merge_page in hotscan?"); */
		cmp_and_merge_page(page, rmap_item);
		/* printk("cmp_and_merge_page not in hotscan"); */
		put_page(page);
	}
}
...
static void remain_zone_scan(unsigned int scan_npages)
{
	...
	list_for_each_entry_continue(rmap_item, &remaining_rmap, link) {
		...
		/* printk("cmp_and_merge_page in remainscan?"); */
		cmp_and_merge_page(page, rmap_item);
		/* printk("cmp_and_merge_page not in remainscan"); */
		put_page(page);
	}
}
/**
...
static int ksm_scan_thread(void *nothing)
{
	...
	while (!kthread_should_stop()) {
		...
		if (ksmd_should_run()) {
			/* if hotscan sleep_ms = 2 else sleep_ms = 20*/
			if(scan_hot_zone) 
				sleep_ms = READ_ONCE(ksm_thread_sleep_millisecs_hotzone);
			else 
...
/* sleep when hotzone scan*/
static ssize_t sleep_millisecs_hotzone_show(struct kobject *kobj,
				    struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", ksm_thread_sleep_millisecs_hotzone);
}

static ssize_t sleep_millisecs_hotzone_store(struct kobject *kobj,
				     struct kobj_attribute *attr,
				     const char *buf, size_t count)
{
	unsigned long msecs;
	...
	ksm_thread_sleep_millisecs_hotzone = msecs;
	wake_up_interruptible(&ksm_iter_wait);

	return count;
}
KSM_ATTR(sleep_millisecs_hotzone);
...
static struct attribute *ksm_attrs[] = {
	...
	&sleep_millisecs_hotzone_attr.attr,
