...
/* HZ */	
static unsigned long get_gfn(struct kvm_memslots *slots, unsigned long hva)
{
	int i;
	unsigned long gfn = 0;
	for(i = 0; i < KVM_MEM_SLOTS_NUM; i++) {
		...
		{
			return gfn = (hva - slotuser) + slots->memslots[i].base_gfn;
		}
    }
	return gfn;
}
...
/* HZ */	
static unsigned long get_number(struct kvm_memslots *slots, unsigned long hva)
{
	int i;
	for(i = 0; i < KVM_MEM_SLOTS_NUM; i++) {
...
unsigned long kvm_hva_vmnumber(unsigned long hva)
{
	...
	list_for_each_entry(kvm_list, &vm_list, vm_list) {
		...
		for(i = 0; i < KVM_ADDRESS_SPACE_NUM; i++) {
			num = get_number(kvm_list->memslots[i], hva);
			if(num != 0)
			{ 
				return num;
			}
